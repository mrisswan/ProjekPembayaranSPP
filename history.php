...
<body>
    ...
    <center>
        ...
        <form action="" method="POST" autocomplete="off">
            ...
            <button type="submit" name="cari" button class="btn btn-dark">Cari</button>
        </form>
    </center>
...
if(isset($_POST['cari'])){
    ...
    $historyPembayaran = mysqli_query($db, "SELECT * FROM pembayaran JOIN petugas ON pembayaran.id_petugas=petugas.id_petugas
                         JOIN spp ON pembayaran.id_spp=spp.id_spp WHERE nisn='$nisn' ORDER BY tgl_bayar");
    ...
    if(mysqli_num_rows($cari) == 0){
        echo "<td><center>NISN belum terdaftar!</center></td>";
    }else{
        echo "";
    }
?>
    ...
    <!-- Kita tampilkan Biodata Siswa -->
        ...
        <table cellspacing="0"  cellpadding="5" class="table table-striped" border="1">
            <tr>
                <td>No. </td>
...
while($r_trx = mysqli_fetch_assoc($historyPembayaran)){ ?>
            <tr>
                <td><?= $no; ?></td>
                <td><?= $r_trx['tgl_bayar'] . " " . $r_trx['bulan_dibayar'] . " " . $r_trx['tahun_dibayar']; ?></td>
...
<?php }else{ ?> <td>BELUM LUNAS</td>
                <td><a href="transaksi.php?lunas&id=<?= $r_trx['id_pembayaran']; ?>">BAYAR LUNAS</a></td>
<?php } ?>
            </tr>
<?php $no++; }?>
        </table>
